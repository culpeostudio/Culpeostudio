...
import 'package:culpeo_studio/modules/engine/controller.dart';
import 'package:culpeo_studio/modules/engine/engine_api.dart';
import 'package:culpeo_studio/modules/engine/models.dart';
import 'package:culpeo_studio/modules/engine/widgets.dart';
import 'package:culpeo_studio/modules/engine/engine_screen.dart';
...
void main() {
  ...
  testWidgets('applies a preset into the form and saves the current settings', (
    tester,
  ) async {
    ...
    final api = _FakeEngineApi()
      ..presetValues = [
        const EnginePreset(
          id: 'builtin_large_model',
          name: 'Großes Modell auf kleiner Karte',
          description: 'Erlaubt System-RAM.',
          builtIn: true,
          config: EngineConfig(
            priority: 'high',
            ...
            runtimeOptions: {'allow_ram_offload': true, 'cpu_moe': true},
          ),
        ),
      ];
    ...
    final open = find.byKey(const Key('engine-open-presets'));
    await tester.ensureVisible(open);
    await tester.tap(open);
    await tester.pumpAndSettle();

    // The strings for this dialog live in the screen's own map, not the global
    // one, so a wrong import would show the raw keys here.
    expect(find.text('Gespeicherte Konfigurationen'), findsOneWidget);
    expect(find.text('Großes Modell auf kleiner Karte'), findsOneWidget);
    expect(find.text('mitgeliefert'), findsOneWidget);
    // A built-in is the fallback, so it offers no delete.
    expect(
      find.byKey(const Key('engine-preset-delete-builtin_large_model')),
      findsNothing,
    );

    await tester.tap(
      find.byKey(const Key('engine-preset-apply-builtin_large_model')),
    );
    await tester.pumpAndSettle();

    // Applying writes into the form rather than starting anything, so the
    // values only reach the engine when the user creates the instance.
    ...
    final created = api.lastCreateConfig;
    expect(created?.priority, 'high');
    expect(created?.gatewayAutostart, isTrue);
    expect(created?.restartOnCrash, isTrue);
    expect(created?.idleTimeoutSeconds, -1);
    // allow_ram_offload is deliberately not asserted here. The calculate step
    // re-plans without RAM first and keeps that plan when the model fits in
    // VRAM, so a preset asking for RAM is a preference the planner is right to
    // overrule - spilling into RAM when it is not needed only costs speed.
    expect(tester.takeException(), isNull);
  });

  testWidgets('saves the current settings as a named preset', (tester) async {
    await _setViewport(tester, const Size(900, 1400));
    final api = _FakeEngineApi();
    ...
    // An unnamed preset has nothing to find it by, so it is refused before it
    // reaches the backend.
    await tester.tap(find.byKey(const Key('engine-preset-save')));
    await tester.pumpAndSettle();
    expect(find.text('Bitte einen Namen eingeben.'), findsOneWidget);
    expect(api.presetValues, isEmpty);

    await tester.enterText(
      find.byKey(const Key('engine-preset-name')),
      'Mein Setup',
    );
    await tester.tap(find.byKey(const Key('engine-preset-save')));
    await tester.pumpAndSettle();
    ...
    expect(
      api.presetValues.map((preset) => preset.name),
      contains('Mein Setup'),
    );
    expect(tester.takeException(), isNull);
  });
  ...
  testWidgets('routes an async GPU runtime fix to the Vulkan runtime only', (
    ...
    final api = _GpuRuntimeRebuildApi([failed]);
...
class _FakeEngineApi implements EngineApi {
  ...
  QuantizationJob? lastQuantizationJob;
  List<EnginePreset> presetValues = const [];
  ...
  Future<InstanceLogs> getEngineInstanceLogs(
    String instanceId, {
    int tailLines = 0,
  }) async => const InstanceLogs();

  @override
  Future<QuantizationCatalog> getQuantizationTypes() async =>
      const QuantizationCatalog(
        available: true,
        types: [QuantizationType(name: 'Q4_K_M')],
      );

  @override
  Future<QuantizationPreflight> preflightQuantization(
    QuantizationJob job,
  ) async => QuantizationPreflight(
  ...
  @override
  Future<EngineQuantizationStart> startQuantization(QuantizationJob job) async {
    lastQuantizationJob = job;
    return const EngineQuantizationStart(operationId: 'op_quantize');
  }

  @override
  Future<List<EnginePreset>> getEnginePresets() async => presetValues;
  ...
  Future<EnginePreset> saveEnginePreset({
    ...
    final saved = EnginePreset(
      id: id.isEmpty ? 'pre_saved' : id,
      name: name,
      description: description,
      config: config,
      modelId: modelId,
    );
    presetValues = [...presetValues, saved];
    return saved;
  }

  @override
  Future<void> deleteEnginePreset(String presetId) async {
  ...
  ]) async => const EnginePresetExport(document: '{"schema_version":1}');

  @override
  Future<List<EnginePreset>> importEnginePresets(String document) async =>
      presetValues;
}
...
class _GpuRuntimeRebuildApi extends _FakeEngineApi {
  _GpuRuntimeRebuildApi(List<EngineInstance> instances)
  ...
  Future<EngineMutationResult> installEngineRuntime(String runtimeId) async {
    ...
    completeOperationSoon('llama-runtime-rebuild', type: 'runtime_install');
    return const EngineMutationResult(operationId: 'llama-runtime-rebuild');
  }
}
