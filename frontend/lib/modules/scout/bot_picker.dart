...
class BotPicker extends StatelessWidget {
  ...
  final List<ScoutChoice> bots;
  final String? selectedBotId;
  final ValueChanged<String?> onSelected;
  final bool enabled;
  final bool? useBottomSheet;

  ScoutChoice? get selectedBot {
  ...
  PopupMenuItem<String> _popupItem(ScoutChoice? bot) {
  ...
  Widget _sheetItem(BuildContext context, ScoutChoice? bot) {
    final id = bot?.id ?? '__auto__';
    final selected =
        bot?.id == selectedBotId || (bot == null && selectedBotId == null);
    return ListTile(
    ...
    return Row(
      children: [
        Icon(
          ...
          color: selected ? CulpeoColors.metricBright : Colors.white54,
          size: 18,
        ),
        ...
        if (selected) ...[
          ...
          const Icon(Icons.check, size: 16, color: CulpeoColors.metricBright),
        ],
      ],
    );
  }
