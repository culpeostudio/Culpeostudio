...
class NewsScreen extends StatefulWidget {
  ...
  static Future<List<dynamic>> _defaultLoadNews() =>
      ApiService().news.getNews();

  static Future<List<dynamic>> _defaultLoadSavedNews() =>
      ApiService().news.getSavedNews();

  static Future<void> _defaultSaveNews(Map<String, dynamic> article) =>
      ApiService().news.saveNewsArticle(article);

  static Future<void> _defaultUnsaveNews(String articleId) =>
      ApiService().news.deleteSavedNewsArticle(articleId);
...
class _NewsScreenState extends State<NewsScreen> {
  ...
  /// Uebernimmt die beim Start vorgeladenen News und gespeicherten Artikel,
  /// sodass die Seite ohne Ladezustand aufgebaut wird. [_fetchNews] und
  /// [_fetchSavedNews] laden danach im Hintergrund nach.
  void _seedFromWarmup() {
    final warmup = StartupWarmup.instance;
    final news = warmup.take<List<dynamic>>(
      'news.list',
      ttl: StartupWarmup.newsTtl,
    );
    if (news != null) {
      setState(() {
        _newsList = news;
    ...
    final saved = warmup.take<List<dynamic>>(
      'news.saved',
      ttl: StartupWarmup.savedNewsTtl,
    );
    if (saved != null) {
      setState(() {
        _savedList = saved;
  ...
  Future<void> _fetchNews() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        // Vorgeladene Artikel bleiben sichtbar; erst ohne Bestand wird der
        // Fehler angezeigt.
  ...
  Future<void> _fetchSavedNews() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        // Vorgeladene gespeicherte Artikel bleiben sichtbar; erst ohne
        // Bestand wird der Fehler angezeigt.
            ...
            child: _isLoading
                ? const Center(
                    child: CircularProgressIndicator(
                      valueColor: AlwaysStoppedAnimation(CulpeoColors.metric),
                    ),
                  )
                : _errorMessage != null
                ? Center(
  ...
  Widget _buildFilterMenu({
    ...
    final borderColor = DarkColors.primary.withValues(alpha: 0.32);
      ...
      children: [
        ...
        PopupMenuButton<String>(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: BorderSide(color: DarkColors.primary.withValues(alpha: 0.52)),
          ),
          onOpened: onOpened,
          onCanceled: onClosed,
        ...
        children: [
          ...
          if (imageUrl != null && imageUrl.isNotEmpty)
            Image.network(
              ...
              headers: _imageRequestHeaders(imageUrl, articleUrl: articleUrl),
  ...
  // Viele Bild-CDs (heise.cloudimg.io, Golem, Vox) lehnen Anfragen ohne
  // Browser-User-Agent oder ohne passenden Referer mit einem 403 ab. Der
  // Dart-Client von Image.network sendet standardmäßig beides nicht, daher
  // schlagen Artikelbilder im Desktop-Client fehl, obwohl sie im Backend
  // sauber extrahiert wurden. Auf dem Web-Client werden diese Header ignoriert.
