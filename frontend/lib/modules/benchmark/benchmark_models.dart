...
class BenchmarkBoard {
  ...
  /// The ceiling of the board itself, used only until the field statistics
  /// arrive — bars are capped by the leader of their category, not by this.
...
/// The best score the field currently reaches, per category and overall.
///
/// Every score bar runs from zero up to this ceiling, so its length reads as
/// "share of the leader's score": a full bar means nobody is better in that
/// category, half a bar means half of what the leader scores.
class BenchmarkFieldTops {
  const BenchmarkFieldTops({this.primary = 0, this.byMetric = const {}});

  factory BenchmarkFieldTops.of(BenchmarkOverview overview) {
    final byMetric = <String, double>{};
    overview.metricStats.forEach((key, stats) {
      if (stats.max > 0) byMetric[key] = stats.max;
    });

    var primary = 0.0;
    for (final entry in overview.topOverall) {
      if (entry.primary > primary) primary = entry.primary;
    }

    return BenchmarkFieldTops(primary: primary, byMetric: byMetric);
  }

  static const BenchmarkFieldTops empty = BenchmarkFieldTops();

  final double primary;
  final Map<String, double> byMetric;

  /// The ceiling of a category, or [fallback] while no statistics have arrived.
  double ceilingOf(String metricKey, {double fallback = 0}) {
    final top = byMetric[metricKey] ?? 0;
    return top > 0 ? top : fallback;
  }

  /// The ceiling of the headline score, or [fallback] without statistics.
  double primaryCeiling({double fallback = 0}) =>
