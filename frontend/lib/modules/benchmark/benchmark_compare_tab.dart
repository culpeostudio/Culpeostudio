...
class _BenchmarkCompareTabState extends State<BenchmarkCompareTab> {
  ...
  Widget _buildColumnHead(int column) {
    ...
    final color = pending ? kBenchmarkInkFaint : _colorOf(column);
  ...
  /// Turns a category name in the label column into the way to its entry in
  /// the wiki; the headline row carries no key and stays plain text.
  Widget _explainable(String metricKey, Widget child) {
    final open = widget.onOpenMetric;
    if (metricKey.isEmpty || open == null) return child;
    ...
    return _CompareRow(
      ...
      cell: (column) {
        ...
        return _buildScoreCell(
          ...
          best: best,
          ceiling: headline
              ? widget.fieldTops.primaryCeiling(fallback: board.scoreMax)
              : widget.fieldTops.ceilingOf(metricKey, fallback: board.scoreMax),
          reference: reference,
  ...
  Widget _buildScoreCell({
    ...
    // Zero to the best score of the field: a full bar means nobody is ahead.
