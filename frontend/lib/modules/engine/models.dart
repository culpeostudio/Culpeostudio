  ...
  factory RuntimeCapability.fromJson(dynamic value) {
    ...
    return RuntimeCapability(
      ...
      buildVersion: _string(
        _first(json, const ['build_version', 'buildVersion']),
      ),
...
class EngineConfig {
  ...
  /// it. Off by default, because a request that silently loads tens of
  /// gigabytes is not something to do unasked.
  final bool gatewayAutostart;
  ...
  /// How long the instance may sit unused before it is unloaded. Null follows
  ...
  factory EngineConfig.fromJson(dynamic value) {
    ...
    return EngineConfig(
      ...
      gatewayAutostart: _boolean(
        _first(json, const ['gateway_autostart', 'gatewayAutostart']),
      ),
      restartOnCrash: _boolean(
        _first(json, const ['restart_on_crash', 'restartOnCrash']),
      ),
      idleTimeoutSeconds:
          _first(json, const ['idle_timeout_seconds', 'idleTimeoutSeconds']) ==
              null
          ? null
          : _integer(
              _first(json, const [
  ...
  JsonMap toJson() => {
    ...
    'autostart': autostart,
    'gateway_autostart': gatewayAutostart,
  ...
  EngineConfig copyWith({
    ...
    int? maxSequences,
    String? priority,
    String? kvCachePolicy,
    bool? allowFallback,
    bool? autostart,
    bool? gatewayAutostart,
    bool? restartOnCrash,
    int? idleTimeoutSeconds,
    JsonMap? runtimeOptions,
    JsonMap? generationDefaults,
  }) => EngineConfig(
    ...
    kvCachePolicy: kvCachePolicy ?? this.kvCachePolicy,
    allowFallback: allowFallback ?? this.allowFallback,
    autostart: autostart ?? this.autostart,
    gatewayAutostart: gatewayAutostart ?? this.gatewayAutostart,
    restartOnCrash: restartOnCrash ?? this.restartOnCrash,
    idleTimeoutSeconds: idleTimeoutSeconds ?? this.idleTimeoutSeconds,
    runtimeOptions: runtimeOptions ?? this.runtimeOptions,
    generationDefaults: generationDefaults ?? this.generationDefaults,
  );
}
...
class EngineInstance {
  ...
  /// The gateway and crash-recovery opt-ins, mirrored off the config so a list
  /// view can render the toggles without reaching into requestedConfig.
  final bool gatewayAutostart;
  final bool restartOnCrash;
  final int? idleTimeoutSeconds;
...
/// One target format the installed llama.cpp build can write. The figures come
/// from that build's own help output rather than from a table kept here, so
/// they always describe the binary that would do the work.
class QuantizationType {
  final String name;
  final String description;
  final double sizeGibAtReference;
  final double perplexityDelta;
  ...
  const QuantizationType({
    required this.name,
    this.description = '',
    this.sizeGibAtReference = 0,
    this.perplexityDelta = 0,
    this.referenceModel = '',
    this.bitsPerWeight = 0,
    this.alias = '',
  });

  /// True for a name that only points at another entry, such as Q4_K to Q4_K_M.
  /// Offering both in a picker would look like two different choices.
  bool get isAlias => alias.isNotEmpty;

  factory QuantizationType.fromJson(dynamic value) {
    final json = _asMap(value);
    return QuantizationType(
      name: _string(json['name']),
      description: _string(json['description']),
      sizeGibAtReference: _decimal(
        _first(json, const ['size_gib_at_reference', 'sizeGibAtReference']),
      ),
      perplexityDelta: _decimal(
        _first(json, const ['perplexity_delta', 'perplexityDelta']),
      ),
      referenceModel: _string(
        _first(json, const ['reference_model', 'referenceModel']),
      ),
      bitsPerWeight: _decimal(
        _first(json, const ['bits_per_weight', 'bitsPerWeight']),
      ),
...
/// What a conversion would do, computed before anything is written. The size is
/// not estimated from bits per weight: it is what the tool's own dry run said.
class QuantizationPreflight {
  final String sourceModelId;
  final String sourceName;
  final String sourceQuantization;
  ...
  final double sourceBitsPerWeight;
  final double targetBitsPerWeight;
  final bool isRequantization;
  final bool feasible;
  final List<String> blockers;
  final List<String> warnings;

  const QuantizationPreflight({
    this.sourceModelId = '',
    this.sourceName = '',
    this.sourceQuantization = '',
    this.sourceBytes = 0,
    this.targetType = '',
    this.targetName = '',
    ...
    this.requiredDiskBytes = 0,
    this.sourceBitsPerWeight = 0,
    this.targetBitsPerWeight = 0,
    this.isRequantization = false,
    this.feasible = false,
    this.blockers = const [],
    this.warnings = const [],
  });

  /// How much smaller the result is, as a fraction. Zero when the sizes are not
  /// known, which is the only case a caller should not show a saving for.
  double get savedFraction {
    if (sourceBytes <= 0 || estimatedBytes <= 0) return 0;
    final saved = (sourceBytes - estimatedBytes) / sourceBytes;
    return saved <= 0 ? 0 : saved;
  }

  factory QuantizationPreflight.fromJson(dynamic value) {
    final json = _asMap(value);
    return QuantizationPreflight(
      sourceModelId: _string(
        _first(json, const ['source_model_id', 'sourceModelId']),
      ),
      sourceName: _string(_first(json, const ['source_name', 'sourceName'])),
      sourceQuantization: _string(
        _first(json, const ['source_quantization', 'sourceQuantization']),
      ),
      sourceBytes: _integer(
        _first(json, const ['source_bytes', 'sourceBytes']),
      ),
      targetType: _string(_first(json, const ['target_type', 'targetType'])),
      targetName: _string(_first(json, const ['target_name', 'targetName'])),
      targetRelativePath: _string(
        _first(json, const ['target_relative_path', 'targetRelativePath']),
      ),
      estimatedBytes: _integer(
        _first(json, const ['estimated_bytes', 'estimatedBytes']),
      ),
      freeDiskBytes: _integer(
        _first(json, const ['free_disk_bytes', 'freeDiskBytes']),
      ),
      requiredDiskBytes: _integer(
        _first(json, const ['required_disk_bytes', 'requiredDiskBytes']),
      ),
      sourceBitsPerWeight: _decimal(
        _first(json, const ['source_bits_per_weight', 'sourceBitsPerWeight']),
      ),
      targetBitsPerWeight: _decimal(
        _first(json, const ['target_bits_per_weight', 'targetBitsPerWeight']),
      ),
      isRequantization: _boolean(
        _first(json, const ['is_requantization', 'isRequantization']),
      ),
      feasible: _boolean(json['feasible']),
...
/// The set of target formats, plus why there are none when a runtime has not
/// been installed yet.
class QuantizationCatalog {
  final List<QuantizationType> types;
  final bool available;
  final String unavailableReason;

  const QuantizationCatalog({
    this.types = const [],
    this.available = false,
    this.unavailableReason = '',
  });

  factory QuantizationCatalog.fromJson(dynamic value) {
    final json = _asMap(value);
    return QuantizationCatalog(
      types: _asList(json['types']).map(QuantizationType.fromJson).toList(),
      available: _boolean(json['available']),
      unavailableReason: _string(
        _first(json, const ['unavailable_reason', 'unavailableReason']),
      ),
    );
  }
}

/// The worker process's own output. Absent for a stopped instance: the buffers
/// live with the process.
class InstanceLogs {
  final String stdout;
  final String stderr;
  final bool available;
  ...
  bool get isEmpty => stdout.trim().isEmpty && stderr.trim().isEmpty;

  factory InstanceLogs.fromJson(dynamic value) {
    final json = _asMap(value);
    return InstanceLogs(
...
/// A saved engine configuration under a name. It is not bound to the model it
/// was saved from: a preset is a set of settings, applicable to any of them.
class EnginePreset {
  final String id;
  final String name;
  final String description;
  ...
  /// Shipped with the engine: applicable and copyable, but never editable, so
  ...
  factory EnginePreset.fromJson(dynamic value) {
    final json = _asMap(value);
    return EnginePreset(
      id: _string(json['id']),
      name: _string(json['name']),
      ...
      modelId: _string(_first(json, const ['model_id', 'modelId'])),
      createdAt: _dateTime(_first(json, const ['created_at', 'createdAt'])),
      updatedAt: _dateTime(_first(json, const ['updated_at', 'updatedAt'])),
      builtIn: _boolean(_first(json, const ['built_in', 'builtIn'])),
    );
  }
}

/// An exported preset file, ready to be written wherever the user wants it.
class EnginePresetExport {
  final String document;
  final String suggestedFilename;

  const EnginePresetExport({
    this.document = '',
    this.suggestedFilename = 'culpeo-engine-presets.json',
  });

  factory EnginePresetExport.fromJson(dynamic value) {
    final json = _asMap(value);
    final suggested = _string(
      _first(json, const ['suggested_filename', 'suggestedFilename']),
    );
    return EnginePresetExport(
      document: _string(json['document']),
      suggestedFilename: suggested.isEmpty
          ? 'culpeo-engine-presets.json'
          : suggested,
    );
  }
}
