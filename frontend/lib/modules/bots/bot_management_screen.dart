...
import './bot_management_strings.dart';
import '../../core/app_state.dart';
import '../../core/top_notification.dart';
import '../scout/chat_model_picker.dart';
...
class BotManagementScreen extends StatefulWidget {
  ...
  const BotManagementScreen({
    super.key,
    this.themeColor = CulpeoColors.metric,
...
class _BotManagementScreenState extends State<BotManagementScreen> {
  ...
  Future<void> _loadBots() async {
    ...
    if (mounted) {
      setState(() {
        ...
        if (_appState.scouts.isNotEmpty) {
          final defaultBot = _appState.scouts.firstWhere(
            (b) => b['is_default'] == true,
            orElse: () => _appState.scouts.first,
          );
          _selectBot(Map<String, dynamic>.from(defaultBot));
        }
      });
  ...
  Future<void> _saveBot() async {
    ...
    setState(() => _isLoading = true);
    final success = await _appState.saveScout(botData);
    if (mounted) {
      ...
      if (success) {
        ...
        final saved = _appState.scouts.firstWhere(
          (b) => b['name'] == name,
          orElse: () => _appState.scouts.first,
        );
  ...
  Future<void> _deleteBot() async {
    ...
    if (confirm == true) {
      setState(() => _isLoading = true);
      final success = await _appState.deleteScout(_selectedBot!['id']);
      if (mounted) {
        setState(() => _isLoading = false);
        if (success) {
          showTopNotification(
        ...
        builder: (context, constraints) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              color: CulpeoColors.panel.withValues(alpha: 0.85),
              borderRadius: BorderRadius.circular(16),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: _isLoading && _appState.scouts.isEmpty
                ? const Center(
                ...
                : ListView.builder(
                    ...
                    itemCount: _appState.scouts.length,
                    itemBuilder: (context, index) {
                      final bot = Map<String, dynamic>.from(
                        _appState.scouts[index],
                      );
                      final isSelected =
                          !_isNewBot &&
                          _selectedBot != null &&
                ...
                if (_isNewBot)
                  TextButton(
                    onPressed: () {
                      if (_appState.scouts.isNotEmpty) {
                        _selectBot(
                          Map<String, dynamic>.from(_appState.scouts.first),
                        );
                      } else {
