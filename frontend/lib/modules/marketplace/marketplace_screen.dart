...
class _MarketplaceScreenState extends State<MarketplaceScreen>
  ...
  /// die Anzeige auf die Netzwerkantwort warten muss. Die Suchergebnisse
  /// uebernimmt [StartupWarmup.take] innerhalb von [_triggerSearch]; der
  /// eigentliche Netzwerkabruf laeuft danach trotzdem weiter und ersetzt
  /// die gecachten Daten mit frischen.
  void _seedFromWarmup() {
    final warmup = StartupWarmup.instance;
    final hardware = warmup.take<Map<String, dynamic>>(
      'marketplace.hardware',
      ttl: StartupWarmup.marketplaceHardwareTtl,
    );
    if (hardware != null) {
      _hardwareProfile = hardware;
      _hardwareError = '';
    }
    final jobs = warmup.take<Map<String, dynamic>>(
      'marketplace.jobs',
      ttl: StartupWarmup.marketplaceJobsTtl,
    );
    if (jobs != null && jobs['jobs'] is List) {
      _downloadJobs = jobs['jobs'] as List;
    }
  }
  ...
  Future<void> _fetchHardwareProfile() async {
    final res = await _api.marketplace.getHardwareProfile();
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _fetchDownloadJobs() async {
    final res = await _api.marketplace.listDownloadJobs();
    if (!mounted) return;
  ...
  Future<void> _triggerSearch({bool collapseSearch = false}) async {
    ...
    final warmSearch = _takeWarmSearch();
    if (warmSearch != null) {
      final refreshGeneration = ++_searchGeneration;
      setState(() {
        _searchResults = warmSearch['models'] as List<dynamic>;
        _hasMore = warmSearch['has_more'] ?? false;
        _page = 1;
        _isSearching = false;
      });
      unawaited(_loadPage(1, refreshGeneration));
      return;
    }
  ...
  /// Nur die Standardansicht (alle Provider, keine Filter) darf die beim
  /// Start vorgeladenen Suchergebnisse anzeigen; sobald der User filtert,
  /// trifft die im Cache genannte Anzeige nicht mehr zu.
  Map<String, dynamic>? _takeWarmSearch() {
    if (_provider != 'all' ||
        _searchController.text.trim().isNotEmpty ||
        _category.isNotEmpty ||
        _sort != 'popularity' ||
        _quantization.isNotEmpty ||
        _gpuFit ||
        _localOnly) {
      return null;
    }
    final res = StartupWarmup.instance.take<Map<String, dynamic>>(
      'marketplace.search',
      ttl: StartupWarmup.marketplaceSearchTtl,
    );
    if (res == null || res['models'] is! List) return null;
    return res;
  }
  ...
  Future<void> _loadPage(int page, int generation) async {
    final res = await _api.marketplace.search(
  ...
  Future<void> _deleteJob(String id) async {
    final res = await _api.marketplace.deleteDownloadJob(id);
    if (!mounted) return;
    if (res.containsKey('error')) {
  ...
  Future<void> _downloadModel(
    ...
    final res = await _api.marketplace.downloadModel(
      provider,
      modelId,
      assetId,
      '',
  ...
  Future<void> _startApiModel(Map<String, dynamic> model) async {
    ...
    final res = await _api.marketplace.startAPIModelForChat(
      provider,
      modelId,
      displayName,
    );
  ...
  Widget _buildToolbar() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: CulpeoColors.panel.withValues(alpha: 0.22),
        border: Border(
          ...
          suffixIcon: Tooltip(
            ...
            child: IconButton(
              ...
              style: IconButton.styleFrom(
                hoverColor: CulpeoColors.action.withValues(alpha: 0.22),
              ),
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: CulpeoColors.metric.withValues(alpha: 0.10),
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(
                      color: CulpeoColors.metric.withValues(alpha: 0.28),
                    ),
                  ),
                  child: Row(
                    crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget _buildFilterLabel() {
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        const Icon(Icons.tune_rounded, size: 15, color: CulpeoColors.metric),
        const SizedBox(width: 6),
        Text(
  ...
  Color _filterOptionColor(String group, String value) {
    ...
    return CulpeoColors.metricBright;
  }
  ...
  Widget _buildModelCard(dynamic model, {required bool compact}) {
    ...
    final accent = cloud ? providerColor : CulpeoColors.metric;
  ...
  Future<void> _showModelDetail(
    ...
    final detail = await _api.marketplace.getModelDetail(modelId, provider);
    if (!mounted) return;
