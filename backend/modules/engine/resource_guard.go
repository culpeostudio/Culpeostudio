...
// loadPeakBytes estimates the transient high-water mark while a model loads.
// llama.cpp reads the whole GGUF through host memory before handing the layers
// to the GPU, so the weights are charged to RAM once even when they end up in
// VRAM, plus headroom for the read buffers.
func loadPeakBytes(record modelcatalog.ModelRecord, plan ContextPlanView) (ram int64, gpu map[string]int64) {
...
func loadPeakReserveAdditions(record modelcatalog.ModelRecord, plan ContextPlanView) (int64, map[string]int64) {
	ramRequired, gpuRequired := loadPeakBytes(record, plan)
...
func allocateWithLoadPeak(
	...
	ramExtra, gpuExtra := loadPeakReserveAdditions(record, planView(target, nil))
	...
	if err := validatePeakAwareAllocation(hardwarePlan, secondPass, basePolicy, targetID, record); err != nil {
		return nil, err
	}
...
func (m *EngineModule) validateLoadPeak(ctx context.Context, record modelcatalog.ModelRecord, plan ContextPlanView) (int64, error) {
	m.mu.RLock()
	guard := m.guardState
	m.mu.RUnlock()
...
func (m *EngineModule) runResourceMaintenance() {
	...
	// The health watch runs far more often than the idle sweep: an instance
	// whose worker died is being advertised as ready in the meantime, and every
	// second it stays that way is a request that fails for no visible reason.
...
func (m *EngineModule) runIdleSweep(now time.Time) []string {
	...
	for _, id := range ids {
		...
		if _, err := m.scheduleStopWithReason(id, "idle_timeout", "Instanz wurde nach Ablauf der Leerlaufzeit ohne Nutzung automatisch entladen"); err == nil {
