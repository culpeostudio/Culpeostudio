...
const marketplaceBundleManifest = ".culpeostudio-complete.json"
...
func (m *EngineModule) deleteModel(ctx context.Context, id string) ([]modelcatalog.ModelRecord, error) {
	...
	stagingDir := ".culpeostudio-delete-" + nonce + ".tmp"
...
func cleanupStagedModelDeletionsRoot(rootFS *os.Root) error {
	...
	for _, entry := range entries {
		name := entry.Name()
		if !entry.IsDir() || !strings.HasPrefix(name, ".culpeostudio-delete-") || !strings.HasSuffix(name, ".tmp") {
			continue
		}
