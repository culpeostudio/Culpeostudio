...
func (m *EngineModule) startRuntimeInstall(ctx context.Context, build engineruntime.Build) (*engineruntime.InstallJob, error) {
	for {
		m.spawnGateMu.Lock()
		...
		job, err := m.installer.Start(build)
		if err != nil {
			m.runtimeInstallMu.Unlock()
			m.spawnGateMu.Unlock()
			return nil, err
...
func (m *EngineModule) scheduleRuntimePrewarm(reason string) {
	...
	candidates := m.prewarmCandidates(snapshot)
	if len(candidates) == 0 {
		return
	}
...
// prewarmCandidates lists the builds worth fetching before a start is asked
// for: the one this hardware wants, and the CPU build behind it so a GPU
// failure has somewhere to land.
func (m *EngineModule) prewarmCandidates(snapshot hardware.Snapshot) []runtimePrewarmCandidate {
	result := []runtimePrewarmCandidate{}
	seen := map[string]bool{}
	add := func(build engineruntime.Build, purpose string) {
		digest, err := build.Digest()
		if err != nil || seen[digest] {
			return
		}
		seen[digest] = true
		result = append(result, runtimePrewarmCandidate{build: build, purpose: purpose})
	}
	if build, _, err := m.selectBuild(snapshot, false); err == nil {
		add(build, prewarmPurpose(build.Variant))
	}
	if build, err := m.selectBuildForCPU(); err == nil {
		add(build, "llama.cpp CPU-Fallback wird vorbereitet")
	}
	return result
}

// prewarmPurpose describes, for the operation log, which build is being fetched.
func prewarmPurpose(variant engineruntime.BuildVariant) string {
	if variant == engineruntime.BuildCPU {
		return "llama.cpp CPU-Runtime wird vorbereitet"
	}
	return fmt.Sprintf("llama.cpp %s-Unterstuetzung wird vorbereitet", strings.ToUpper(string(variant)))
}
...
func (m *EngineModule) runRuntimePrewarm(reason string, candidates []runtimePrewarmCandidate) {
	...
	for _, candidate := range candidates {
		digest, err := candidate.build.Digest()
		if err != nil {
			continue
		}
		m.mu.RLock()
...
func (m *EngineModule) runtimePrewarmLoop() {
	...
	for {
		...
		job, startErr := m.startRuntimeInstall(context.Background(), queued.candidate.build)
		active.job = job
		m.prewarmMu.Unlock()
...
func runtimeInstallProgress(status engineruntime.InstallStatus) float64 {
	...
	case engineruntime.InstallQueued:
		return 0.05
	case engineruntime.InstallDownloading:
		return 0.4
	case engineruntime.InstallVerifying:
		return 0.78
	case engineruntime.InstallExtracting:
		return 0.85
