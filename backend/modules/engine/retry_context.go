...
func effectiveEngineConfig(requested EngineConfig, effective engineruntime.EffectiveConfig, plan ContextPlanView) EngineConfig {
	...
	setOptionalInt(result.RuntimeOptions, "tensor_parallel_size", effective.TensorParallelism)
	if effective.CPUOffloadGB != nil {
		result.RuntimeOptions["cpu_offload_gb"] = *effective.CPUOffloadGB
	}
	...
	if effective.GPUMemoryUtilization != nil {
		result.RuntimeOptions["gpu_memory_utilization"] = *effective.GPUMemoryUtilization
	} else {
		delete(result.RuntimeOptions, "gpu_memory_utilization")
	}
