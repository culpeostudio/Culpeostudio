...
func (m *EngineModule) createInstanceWithID(ctx context.Context, instanceID, modelID, servedName string, config EngineConfig) (*EngineInstance, *EngineOperation, error) {
	...
	instance := &EngineInstance{
		...
		GatewayAutostart: config.GatewayAutostart, RestartOnCrash: config.RestartOnCrash,
		IdleTimeoutSeconds:    cloneIntPointer(config.IdleTimeoutSeconds),
...
func (m *EngineModule) scheduleStartForModel(instanceID, targetModelID string, config EngineConfig, operationType string) (*EngineOperation, error) {
	...
	instance.GatewayAutostart = config.GatewayAutostart
	instance.RestartOnCrash = config.RestartOnCrash
	instance.IdleTimeoutSeconds = cloneIntPointer(config.IdleTimeoutSeconds)
	if !wasReady {
