...
func (m *EngineModule) recordInferenceSample(instanceID string, start, firstToken time.Time, tokens int) {
	...
	m.recordInferenceSampleDirect(instanceID, sample)
}

// recordInferenceSampleDirect files an already-measured sample. The gateway
// uses it because it reads the worker's own usage numbers rather than timing
// the emit callback the way the Studio chat path does.
func (m *EngineModule) recordInferenceSampleDirect(instanceID string, sample inferenceSample) {
