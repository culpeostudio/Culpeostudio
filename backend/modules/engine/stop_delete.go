...
func (m *EngineModule) validateRemoteCode(record modelcatalog.ModelRecord, config EngineConfig) error {
	if !config.TrustRemoteCode || record.Format != modelcatalog.FormatSafeTensors {
		return nil
	}
	root, err := m.modelPath(record)
	if err != nil {
		return err
	}
	pythonHash, count, err := hashPythonFiles(root)
	if err != nil {
		return err
	}
	if count == 0 || m.trust.approved(record.Fingerprint, pythonHash) {
		return nil
	}
	return &trustRequiredError{Fingerprint: record.Fingerprint, PythonHash: pythonHash, FileCount: count}
}

func (m *EngineModule) approveRemoteCode(modelID string) (map[string]interface{}, error) {
	...
	hash, count, err := hashPythonFiles(root)
	if err != nil {
		return nil, err
	}
	if err := m.trust.approve(record.Fingerprint, hash); err != nil {
		return nil, err
	}
	return map[string]interface{}{"model_id": modelID, "fingerprint": record.Fingerprint, "python_files_hash": hash, "python_file_count": count, "warning": "Der Kindprozess ist keine Sandbox; Zustimmung gilt nur fuer diese Hashes."}, nil
}
