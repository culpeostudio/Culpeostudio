...
type EngineConfig struct {
	...
	// Autostart loads the instance when the backend starts.
	Autostart bool `json:"autostart"`

	// GatewayAutostart lets the local OpenAI gateway warm this instance up when
	// a request arrives for it, instead of answering "not ready". It is off by
	// default and stays a deliberate choice: a request that silently loads a
	// 40 GB model is not something to do behind the user's back.
	GatewayAutostart bool `json:"gateway_autostart"`

	// RestartOnCrash lets the health watch bring the instance back after its
	// worker process died on its own. Also opt-in, and also for a reason: a
	// model that crashes on every load should stop, not loop.
	RestartOnCrash bool `json:"restart_on_crash"`

	// IdleTimeoutSeconds overrides how long the instance may sit unused before
	// it is unloaded. Unset follows the engine default; a negative value keeps
	// the instance loaded until it is stopped by hand.
...
// restartOnCrashEnabled reads the flag off a config without caring which of the
// two copies (requested or effective) it came from.
func restartOnCrashEnabled(config EngineConfig) bool {
...
func cloneEngineConfig(value EngineConfig) EngineConfig {
	...
	result.IdleTimeoutSeconds = cloneIntPointer(value.IdleTimeoutSeconds)
