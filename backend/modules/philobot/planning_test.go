...
func (f *fakeModel) turn(convo []chatMessage, systemPrompt string, filterEmit func(string) error) (string, error) {
	f.prompts = append(f.prompts, systemPrompt)
	f.convos = append(f.convos, append([]chatMessage{}, convo...))
	reply := "fertig"
