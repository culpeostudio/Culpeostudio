...
func (s *DownloadJobStore) load() {
	...
	if err := json.Unmarshal(data, &pd); err != nil {
		log.Printf("[marktplatz] download_jobs unmarshal: %v", err)
		return
	}
...
func (s *DownloadJobStore) save() {
	...
	if err != nil {
		log.Printf("[marktplatz] download_jobs marshal: %v", err)
		return
	}
	...
	if dir != "." && dir != "" {
		if err := os.MkdirAll(dir, 0o755); err != nil {
			log.Printf("[marktplatz] download_jobs mkdir %s: %v", dir, err)
			return
		}
	}
	...
	if err := os.WriteFile(tmpPath, data, 0o644); err != nil {
		log.Printf("[marktplatz] download_jobs write tmp: %v", err)
		return
	}
