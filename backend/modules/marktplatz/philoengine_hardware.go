...
const (
	philoEngineHardwarePythonEnv = "PHILOENGINE_HARDWARE_PYTHON"
	philoEngineHardwareProbeEnv  = "PHILOENGINE_HARDWARE_PROBE_PATH"
	legacyWhichLLMPythonEnv      = "WHICHLLM_PYTHON"
	legacyWhichLLMProbeEnv       = "WHICHLLM_PROBE_PATH"
)

type philoEngineProbeGPU struct {
...
type philoEngineProbeResult struct {
	OS            string                `json:"os"`
	...
	GPUs          []philoEngineProbeGPU `json:"gpus"`
...
func detectHardwareProfileWithPhiloEngine() (HardwareProfile, error) {
	...
	defer cancel()
	command := exec.CommandContext(ctx, python, philoEngineProbePath())
	output, err := command.Output()
	if ctx.Err() != nil {
		return HardwareProfile{}, fmt.Errorf("PhiloEngine hardware detection timed out")
	}
	if err != nil {
		return HardwareProfile{}, fmt.Errorf("PhiloEngine hardware detection: %w", err)
	}

	var result philoEngineProbeResult
	if err := json.Unmarshal(output, &result); err != nil {
		return HardwareProfile{}, fmt.Errorf("invalid PhiloEngine hardware JSON: %w", err)
	}
	if strings.TrimSpace(result.Error) != "" {
		return HardwareProfile{}, fmt.Errorf("PhiloEngine hardware detection failed: %s", result.Error)
	}
	if result.RAMBytes <= 0 {
		return HardwareProfile{}, fmt.Errorf("PhiloEngine hardware detection did not report RAM")
	}
	...
	profile := HardwareProfile{
		...
		Detected:        true,
		DetectionSource: "philoengine_hardware",
	...
	for _, gpu := range result.GPUs {
		...
		entry := DetectedGPU{
			Name:                normalizePhiloEngineGPUName(gpu.Vendor, gpu.Name),
...
func philoEnginePythonExecutable() (string, error) {
	if configured := firstNonEmpty(
		os.Getenv(philoEngineHardwarePythonEnv),
	...
	return "", fmt.Errorf("PhiloEngine hardware Python interpreter not found")
}

func philoEngineProbePath() string {
	if configured := firstNonEmpty(
		os.Getenv(philoEngineHardwareProbeEnv),
