...
func TestBotStoreMigratesLegacyArrayForEveryExistingUser(t *testing.T) {
	...
	if stored.Version != StoreSchemaVersion || len(stored.Users) != 2 {
...
func TestBotStoreGivesPendingLegacyOnlyToFirstUser(t *testing.T) {
	...
	legacy := []Config{{ID: "old", Name: "Old", SystemPrompt: "old"}}
	...
	if _, ok := store.GetBotForUser("second", "scout"); !ok {
...
func TestBotStoreValidatesAndNormalizesModelBindings(t *testing.T) {
	store := NewStore(filepath.Join(t.TempDir(), "bots.json"))
	if err := store.EnsureUser("alice"); err != nil {
		t.Fatal(err)
	}
	local := Config{ID: "local", Name: "Local", SystemPrompt: "x", ModelBinding: &ModelBinding{Kind: "local", InstanceID: "inst-1"}}
	...
	invalid := Config{ID: "bad", Name: "Bad", SystemPrompt: "x", ModelBinding: &ModelBinding{Kind: "api", Provider: "unsupported", ModelID: "x"}}
...
func TestBotStoreUserNamespacesDoNotCollideAfterLoginNormalization(t *testing.T) {
	...
	if err := store.SaveBotForUser("alice", Config{ID: "plain-only", Name: "Plain", SystemPrompt: "plain"}); err != nil {
		t.Fatal(err)
	}
	if err := store.EnsureUser("alice!"); err != nil {
		t.Fatal(err)
	...
	if err := store.SaveBotForUser("alice!", Config{ID: "punctuation-only", Name: "Punctuation", SystemPrompt: "punctuation"}); err != nil {
		t.Fatal(err)
	}
	...
	if err := store.SaveBotForUser("张三", Config{ID: "unicode-only", Name: "Unicode", SystemPrompt: "unicode"}); err != nil {
		t.Fatal(err)
	}
