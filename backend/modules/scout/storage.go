...
func (s *sessionStorage) writeRaw(id string, payload []byte) error {
	...
	if id == "" {
		return errors.New("scout: session id required")
	}
	...
	if err := os.WriteFile(tmp, payload, 0o644); err != nil {
		return fmt.Errorf("write scout session: %w", err)
	}
	if err := os.Rename(tmp, s.sessionPath(id)); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("replace scout session: %w", err)
	}
	return nil
}

func (s *sessionStorage) LoadAll() ([]*scoutSession, error) {
	if !s.enabled() {
		return nil, nil
	}
	if err := s.Initialize(); err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("glob scout sessions: %w", err)
	}
	sessions := make([]*scoutSession, 0, len(matches))
	for _, match := range matches {
		content, readErr := os.ReadFile(match)
		if readErr != nil {
			log.Printf("[scout] Session-Datei %s nicht lesbar: %v", match, readErr)
			continue
		}
		var session scoutSession
		if err := json.Unmarshal(content, &session); err != nil {
			log.Printf("[scout] Session-Datei %s nicht dekodierbar: %v", match, err)
			continue
		}
...
func (s *sessionStorage) Delete(id string) error {
	...
	if err := os.Remove(s.sessionPath(strings.TrimSpace(id))); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("delete scout session: %w", err)
	}
	return nil
}

func (m *ScoutModule) persistSession(sessionID string) {
	...
	m.mu.Unlock()
	if err != nil {
		log.Printf("[scout] Session %s serialisieren fehlgeschlagen: %v", sessionID, err)
		return
	}
	if err := m.storage.writeRaw(id, payload); err != nil {
		log.Printf("[scout] Session %s speichern fehlgeschlagen: %v", sessionID, err)
	}
}

type scoutSessionSummary struct {
