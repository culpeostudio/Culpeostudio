...
func buildRuntimeSystemPrompt(base, thinking, style string) string {
	return buildBotRuntimeSystemPrompt(bots.Config{SystemPrompt: base}, thinking, style)
}

const scoutMemoryPreamble = "Langzeit-Gedaechtnis dieses Nutzers (aus frueheren Unterhaltungen). Nutze diese Fakten nur, wenn sie fuer die aktuelle Frage relevant sind; erfinde nichts hinzu und widersprich ihnen nicht ungefragt:"

func (m *ScoutModule) appendMemoryRecall(userID, project, message, systemPrompt string) string {
	if m.agent == nil {
		return systemPrompt
	}
	start := time.Now()
	recall := strings.TrimSpace(m.agent.ProjectMemoryContext(userID, project, message))
	...
	if recall == "" {
		log.Printf("[scout] Memory-Recall leer (user=%s, %s, %s)", userID, scope, elapsed)
		return systemPrompt
	}

	log.Printf("[scout] Memory-Recall injiziert (user=%s, %s, %d Zeichen, %s)", userID, scope, len(recall), elapsed)
	return systemPrompt + "\n\n" + scoutMemoryPreamble + "\n" + recall
}

func buildBotRuntimeSystemPrompt(bot bots.Config, thinking, style string) string {
...
func styleInstruction(style string) string {
	switch bots.NormalizeResponseStyle(style) {
	case "short":
...
func (m *ScoutModule) botBuilderSystemPrompt(userID, base string) string {
	var builder strings.Builder
	builder.WriteString(base)
	builder.WriteString("\n\nAktuelle Bots in der Verwaltung:\n")
	for _, bot := range m.botStore.GetBotsForUser(userID) {
		builder.WriteString(fmt.Sprintf("- id=%s; name=%s; default=%t; style=%s; keywords=%s\n", bot.ID, bot.Name, bot.IsDefault, bots.NormalizeResponseStyle(bot.ResponseStyle), strings.Join(bot.Keywords, ", ")))
	}
