...
func newExecutor(roots []string) (*Executor, error) {
	return NewExecutor(context.Background(), roots, nil, nil, "")
}
...
func (e *Executor) normalizeRoots(roots []string) ([]string, error) {
...
func (e *Executor) resolvePath(rawPath string, allowMissing bool) (string, error) {
...
func (e *Executor) Execute(name string, args map[string]interface{}) map[string]interface{} {
	if args == nil {
...
func (e *Executor) requestPermission(tool, resolved string) string {
	...
	log.Printf("[spark] Permission-Entscheidung (tool=%s, path=%s, decision=%s)", tool, resolved, decision)
	if e.emitEvent != nil {
...
func (e *Executor) grantSessionRoot(resolved string) {
	root := resolved
	if info, err := os.Stat(resolved); err != nil || !info.IsDir() {
...
func (e *Executor) dispatch(name string, args map[string]interface{}) (map[string]interface{}, error) {
	switch name {
	case "list_dir":
...
func (e *Executor) emitFileChanged(resolvedPath, action string, oldText, newText string) {
	if e.emitEvent == nil {
		return
	}
...
func (e *Executor) markRead(resolved string) {
	e.readPaths[strings.ToLower(filepath.Clean(resolved))] = struct{}{}
}

func (e *Executor) wasRead(resolved string) bool {
	_, ok := e.readPaths[strings.ToLower(filepath.Clean(resolved))]
	return ok
}

func (e *Executor) listDir(args map[string]interface{}) (map[string]interface{}, error) {
...
func (e *Executor) readFile(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
	resolved, err := e.resolvePath(path, false)
	if err != nil {
		return nil, err
...
func (e *Executor) writeFile(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
...
func (e *Executor) patchFile(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
...
func (e *Executor) deletePath(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
	resolved, err := e.resolvePath(path, false)
	if err != nil {
		return nil, err
...
func (e *Executor) movePath(args map[string]interface{}) (map[string]interface{}, error) {
...
func (e *Executor) makeDir(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
	resolved, err := e.resolvePath(path, true)
	if err != nil {
		return nil, err
...
func (e *Executor) statPath(args map[string]interface{}) (map[string]interface{}, error) {
	path, _ := args["path"].(string)
	resolved, err := e.resolvePath(path, false)
	if err != nil {
		return nil, err
...
// Roots returns the folders this executor is allowed to touch.
func (e *Executor) Roots() []string { return e.roots }
