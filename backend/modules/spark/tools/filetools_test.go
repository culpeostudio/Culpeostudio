...
func newPermissionTestSetup(t *testing.T, decisions ...string) (*Executor, *fakePermissionAsker, string) {
	t.Helper()
	root := t.TempDir()
	...
	exec, err := NewExecutor(context.Background(), []string{root}, asker, nil, "sess-1")
	if err != nil {
		t.Fatalf("NewExecutor: %v", err)
	}
