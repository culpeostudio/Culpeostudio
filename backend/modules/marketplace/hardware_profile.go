...
func detectHardwareProfileFresh() HardwareProfile {
	...
	profile, culpeoStudioErr := detectHardwareProfileWithCulpeoStudio()
	if culpeoStudioErr != nil {
		profile = HardwareProfile{}
	}
	...
	if snapshot.RAMTotalBytes > 0 {
		...
		if culpeoStudioErr == nil {
			profile.DetectionSource = "culpeostudio_hardware+native_live"
		} else {
...
func detectWindowsGPUDxdiag() (string, int64) {
	tmpDir, err := os.MkdirTemp("", "marketplace-dxdiag-*")
	if err != nil {
		return "", 0
	}
