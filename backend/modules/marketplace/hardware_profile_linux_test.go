...
func TestNormalizeCulpeoStudioGPUNameEnrichesNewAMDDeviceID(t *testing.T) {
	if got := normalizeCulpeoStudioGPUName("amd", "Device 7550"); got != "AMD Radeon RX 9070 XT" {
		t.Fatalf("unexpected enriched name: %q", got)
	}
	if got := normalizeCulpeoStudioGPUName("amd", "AMD Radeon RX 7900 XTX"); got != "AMD Radeon RX 7900 XTX" {
