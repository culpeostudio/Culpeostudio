...
func runLimitedWorker(payload resourceLauncherPayload, maximum uint64) (int, error) {
	...
	lifetime := os.NewFile(uintptr(fd), "culpeostudio-backend-lifetime")
	if lifetime == nil {
		return 126, errors.New("invalid backend-lifetime descriptor")
	}
	defer lifetime.Close()
	unix.CloseOnExec(fd)
	if err := verifyBackendLifetime(fd); err != nil {
		return 126, err
	}
...
func verifyBackendLifetime(fd int) error {
	if err := unix.SetNonblock(fd, true); err != nil {
		return fmt.Errorf("configure backend-lifetime descriptor: %w", err)
	}
	buffer := []byte{0}
	count, err := unix.Read(fd, buffer)
	if restoreErr := unix.SetNonblock(fd, false); restoreErr != nil {
		return fmt.Errorf("restore blocking backend-lifetime descriptor: %w", restoreErr)
	}
	switch {
	case count > 0:
		return errors.New("unexpected data on backend-lifetime descriptor")
	case err == nil:
		return errors.New("backend lifetime ended before worker spawn")
	case errors.Is(err, unix.EAGAIN) || errors.Is(err, unix.EWOULDBLOCK):
		return nil
	default:
		return fmt.Errorf("probe backend-lifetime descriptor: %w", err)
	}
}
