...
const (
	installerLifetimeFlag       = "CULPEOSTUDIO_INSTALLER_LIFETIME"
	installerLifetimeCommand    = "CULPEOSTUDIO_INSTALLER_COMMAND"
	installerLifetimeFD         = "CULPEOSTUDIO_INSTALLER_LIFETIME_FD"
...
func init() {
	...
	if decodeErr != nil || jsonErr != nil || payload.Path == "" || len(payload.Args) == 0 {
		_, _ = fmt.Fprintln(os.Stderr, "invalid Culpeo Studio installer-lifetime configuration")
...
func runInstallerLifetimeSupervisor(payload installerLifetimePayload) (int, error) {
	...
	lifetime := os.NewFile(uintptr(fd), "culpeostudio-installer-backend-lifetime")
	if lifetime == nil {
