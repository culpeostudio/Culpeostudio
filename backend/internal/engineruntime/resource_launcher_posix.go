...
const (
	resourceLauncherFlag       = "CULPEOSTUDIO_RESOURCE_LAUNCHER"
	resourceLauncherCommand    = "CULPEOSTUDIO_RESOURCE_COMMAND"
	resourceLauncherLimit      = "CULPEOSTUDIO_RESOURCE_MEMORY_MAX"
	resourceLauncherLifetimeFD = "CULPEOSTUDIO_RESOURCE_LIFETIME_FD"
	resourceLauncherExecFlag   = "CULPEOSTUDIO_RESOURCE_EXEC"
)
...
func init() {
	...
	if limitErr != nil || decodeErr != nil || jsonErr != nil || maximum == 0 || payload.Path == "" || len(payload.Args) == 0 {
		_, _ = fmt.Fprintln(os.Stderr, "invalid Culpeo Studio resource-launcher configuration")
		os.Exit(126)
	}
	if execLimited {
