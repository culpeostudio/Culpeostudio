...
func (s *Scanner) Scan(ctx context.Context) ([]ModelRecord, error) {
	...
	var safeFiles []discoveredFile
	indexDirs := make(map[string]struct{})
	configDirs := make(map[string]struct{})
	err = filepath.WalkDir(root, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		file := discoveredFile{abs: path, rel: filepath.ToSlash(rel)}
		switch {
	...
	safeGroups := make(map[string][]discoveredFile)
	for _, file := range safeFiles {
		dir := nearestModelDirectory(root, filepath.Dir(file.abs))
		safeGroups[dir] = append(safeGroups[dir], file)
	}
	for dir := range indexDirs {
		if _, exists := safeGroups[dir]; !exists {
			safeGroups[dir] = nil
		}
	}
	for dir := range configDirs {
		if _, exists := safeGroups[dir]; !exists {
			safeGroups[dir] = nil
		}
	}
	for dir, files := range safeGroups {
		records = append(records, scanSafeTensorsDirectory(root, dir, files))
	}

	sort.Slice(records, func(i, j int) bool {
		if records[i].RelativePath == records[j].RelativePath {
			return records[i].Format < records[j].Format
		}
...
func nearestModelDirectory(root, start string) string {
	for dir := filepath.Clean(start); withinRoot(root, dir); dir = filepath.Dir(dir) {
		if regularFileWithinRoot(root, filepath.Join(dir, "config.json")) || regularFileWithinRoot(root, filepath.Join(dir, "model.safetensors.index.json")) {
			return dir
		}
		if dir == root {
			break
...
func regularFileWithinRoot(root, path string) bool {
