...
// Allow reports whether another event is permitted for key and records it when
// it is. It is transport neutral, so the HTTP middleware and the gRPC
// interceptor draw on the same budget.
...
func (r *RateLimiter) Middleware() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if !r.Allow(c.IP() + ":" + strings.TrimSpace(userID)) {
