...
import (
	...
	"github.com/fillyengine/backend/internal/memorychat"
)
...
func renderToolNarrative(arguments, result map[string]interface{}, success bool) string {
	parts := []string{}
	if len(arguments) > 0 {
		payload, _ := json.Marshal(arguments)
		parts = append(parts, "args="+string(payload))
	}
	if len(result) > 0 {
		payload, _ := json.Marshal(result)
		parts = append(parts, "result="+string(payload))
	}
