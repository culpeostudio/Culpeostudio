...
func bundledHardwareProbe(root string) string {
	for _, name := range []string{
		"culpeostudio_hardware_probe.py",
...
func RunBundle(
	...
	managedEnvironment := map[string]string{
		"CULPEOSTUDIO_INSTALL_ROOT": installRoot,
		"CULPEOSTUDIO_VERSION":      bundle.State.Version,
	}
	...
	if hardwareProbe := bundledHardwareProbe(bundle.Root); hardwareProbe != "" {
		managedEnvironment["CULPEOSTUDIO_HARDWARE_PROBE_PATH"] = hardwareProbe
