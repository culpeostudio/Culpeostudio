// Command culpeo-updater is the launcher. It updates a source checkout by
...
import (
	...
	"github.com/culpeohq/backend/internal/autoupdate"
)
...
func runSource(arguments []string, stdout, stderr io.Writer) int {
	...
	if updateDisabled() {
		_, _ = fmt.Fprintln(stdout, "[Updater] Quellcode-Update wurde über CULPEOSTUDIO_SKIP_UPDATE übersprungen.")
		return 0
	}
...
func runCompiled(arguments []string, stdout, stderr io.Writer) int {
	...
	if activeBundle == nil {
		_, _ = fmt.Fprintln(stderr, "Keine startfähige CulpeoStudio-Version gefunden.")
		return 1
	}
...
func updateDisabled() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("CULPEOSTUDIO_SKIP_UPDATE"))) {
	case "1", "true", "yes", "on":
		return true
	default:
		return false
...
func printUsage(writer io.Writer) {
	_, _ = fmt.Fprintln(writer, `CulpeoStudio Updater

Verwendung:
  culpeo-updater source --root <Projektordner>
  culpeo-updater compiled [--root <Installationsordner>] [--check]
  culpeo-updater version

CULPEOSTUDIO_SKIP_UPDATE=1 überspringt die Netzprüfung, startet aber eine
