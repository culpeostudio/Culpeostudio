#!/usr/bin/env python3
"""Build reproducible Culpeo Studio update and first-install archives.
...
TARGETS: dict[str, TargetSpec] = {
    "linux-x64": TargetSpec(
        ...
        backend_name="culpeostudio-server",
        launcher_name="culpeostudio",
        frontend_entrypoint="culpeostudio",
    ...
    "windows-x64": TargetSpec(
        ...
        archive_format="zip",
        backend_name="culpeostudio-server.exe",
        launcher_name="culpeostudio.exe",
        frontend_entrypoint="culpeostudio.exe",
    ...
    "macos-x64": TargetSpec(
        ...
        backend_name="culpeostudio-server",
        launcher_name="culpeostudio",
        frontend_entrypoint="culpeostudio.app/Contents/MacOS/culpeostudio",
        flutter_output="build/macos/Build/Products/Release/culpeostudio.app",
    ),
...
# The engine downloads its own llama-server binary at runtime, so nothing of the
# inference runtime is shipped in the bundle.
BACKEND_RUNTIME_FILES = (
    (
        "tools/culpeostudio_hardware_probe.py",
...
def _build_launcher(destination: Path, target: TargetSpec, version: str) -> None:
    source = PROJECT_ROOT / "backend" / "cmd" / "culpeo-updater"
    if not source.is_dir():
        raise ReleaseBuildError(
            "Launcher source is missing at backend/cmd/culpeo-updater. "
...
def build_release(arguments: BuildArguments) -> dict[str, Any]:
    ...
    update_name = f"culpeostudio-{version}-{target.key}{target.extension}"
    installer_name = (
        f"culpeostudio-{version}-{target.key}-quickinstall{target.extension}"
    )
    ...
    with tempfile.TemporaryDirectory(
        ...
        write_json_atomic(
            bundle_path / ".culpeostudio-bundle.json",
...
def parse_arguments(argv: Sequence[str] | None = None) -> BuildArguments:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build a Culpeo Studio update archive and a self-updating first-install "
