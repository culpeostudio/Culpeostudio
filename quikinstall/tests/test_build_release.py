...
class ContractTests(unittest.TestCase):
    ...
    def test_linux_asset_matches_updater_schema(self) -> None:
        ...
        self.assertEqual(
            asset,
            {
                ...
                "format": "tar.gz",
                "launcher": {
                    "path": "launcher/culpeostudio",
                    "args": [],
                },
                "backend": {
                    "path": "backend/culpeostudio-server",
                    "args": [],
                },
                "frontend": {
                    "path": "frontend/culpeostudio",
                    "args": [],
                },
    ...
    def test_all_platforms_have_expected_archive_and_entrypoint_layout(self) -> None:
        expected = {
            ...
            "macos-x64": (
                ...
                "culpeostudio",
                "culpeostudio-server",
                "culpeostudio.app/Contents/MacOS/culpeostudio",
            ),
            "macos-arm64": (
                "tar.gz",
                "culpeostudio",
    ...
    def test_backend_payload_includes_only_required_runtime_files(self) -> None:
        paths = build_release.backend_payload_paths(
            build_release.TARGETS["linux-x64"],
            self._checkout_with_probe("culpeostudio_hardware_probe.py"),
        )
        self.assertEqual(
            paths,
            (
                "backend/culpeostudio-server",
                "backend/tools/culpeostudio_hardware_probe.py",
    ...
    def test_backend_payload_accepts_the_former_hardware_probe_name(self) -> None:
        ...
        self.assertNotIn("backend/tools/culpeostudio_hardware_probe.py", paths)

    def test_backend_payload_reports_a_missing_hardware_probe(self) -> None:
        temporary = tempfile.TemporaryDirectory()
        self.addCleanup(temporary.cleanup)
        root = Path(temporary.name)
        for relative_path in ("requirements-hardware.txt",):
            source = root / "backend" / relative_path
            source.parent.mkdir(parents=True, exist_ok=True)
            source.write_text("", encoding="utf-8")
        with self.assertRaises(build_release.ReleaseBuildError) as caught:
